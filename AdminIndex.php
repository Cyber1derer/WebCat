...
?>
 <?php if (isset($_COOKIE['login_us'])) {
                                $login=$_COOKIE["login_us"]; 
                                $query2 = "SELECT * FROM account WHERE Adminka='1'";
                                $result =mysqli_query($link,$query2) or die("Ошибка проверки прав. Тебе сюда нельзя" . mysqli_error($link));
                                if($result){ ?>
        ...
        <div class="wrapper">
            ...
            <ul id="block-sidebar">
                <li> <?php if (isset($_COOKIE['login_us'])) {
                                ...
                                if($result){ ?>
                                  <a href="#">  <img src="Content/Admin.jpg"  alt="Смотреть не нужно - нужно покупать" > </a> 
                         <?php   }
                            } ?>
                </li>
...
// Конец функции 
                    echo '
                        ...
                        <p class="style-title-grid"> <a href="AdminContent.php?id='.$row["products_id"].'">'.$row["title"].' </a> </p>';
                         if ($row["Visible"]==0) { 
                            echo '   <p><strong> Товар скрыт </strong> </p>';
                        }
                        echo '
        ...
        <?php   }
                    ...
                    echo 'Ошибка проверки прав. Тебе сюда нельзя';
                }
                            }
                            else{
                            echo 'Ты даже не авторизирован, а пытаешься стать админом....';} ?>
             <!-- Нижняя часть-->
        <div id="footer">
