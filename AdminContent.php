...
// Конец функции 
                      ...
                      ?>  <form action="checkbox-form.php" method="POST">
                        Отображать продукт для рядовых покупателей?
                        <?php if($row["Visible"]){ ?>
                         <input type="checkbox" name="vision" value="1" checked="checked" />
                     <?php   }
                              else{?>
                                 <input type="checkbox" name="vision" value="0" /> 
                                 <?php  }?>
                         <input type="submit" name="formSubmit" value="Применить" />
                     </form> <?php
                    }
...
VK.Widgets.Comments("vk_comments2", {limit: 10, width: "850", attach: "*"});
        ...
        <?php   }
                    ...
                    echo 'Ошибка проверки прав. Тебе сюда нельзя';
                            ...
                            else {  echo 'Ты даже не авторизирован, а пытаешься стать админом....'; } ?>
             <!-- Нижняя часть--> 
        <div id="footer">
